...
const CountryPhoneSelect = ({ country, phone, onCountryChange, onPhoneChange }) => {
  ...
  const selected = COUNTRIES.find(c => c.name === localCountry) || null;
  ...
  const handleSelect = (c) => {
    setLocalCountry(c.name);
    onCountryChange(c.name);
    onPhoneChange(c.dial + ' ');
    setOpen(false);
    setSearch('');
  };

  // Extract local number (strip dial code prefix)
  const localNumber = selected
    ? phone.startsWith(selected.dial) ? phone.slice(selected.dial.length).trimStart() : phone
    : phone;

  const handlePhoneInput = (e) => {
    const val = e.target.value;
    onPhoneChange(selected ? selected.dial + ' ' + val : val);
  };
  ...
  return (
    <div className="cps-wrap">
      ...
      <div className="form-group">
        <label>Country</label>
        <div className="cps-country-field" ref={dropRef}>
          <button type="button" className="cps-trigger" onClick={() => setOpen(!open)}>
            {selected
              ? <span className={`fi fi-${selected.code.toLowerCase()} cps-flag-img`}></span>
              : <span className="cps-flag-globe">🌍</span>
            }
            <span className="cps-name">{selected ? selected.name : 'Select country'}</span>
            {selected && <span className="cps-trigger-dial">{selected.dial}</span>}
            <span className="cps-arrow">{open ? '▲' : '▼'}</span>
          </button>
          ...
          {open && (
            <div className="cps-dropdown">
              ...
              <div className="cps-list">
                {filtered.length === 0 && (
                  <div className="cps-empty">No countries found</div>
                )}
                {filtered.map(c => (
                  <div
                    key={c.code}
                    className={`cps-item ${selected?.code === c.code ? 'active' : ''}`}
                    onClick={() => handleSelect(c)}
                  >
                    <span className={`fi fi-${c.code.toLowerCase()} cps-flag-img`}></span>
                    <span className="cps-item-name">{c.name}</span>
                    <span className="cps-item-dial">{c.dial}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      <div className="form-group">
        ...
        <div className="cps-phone-wrap">
          {selected && (
            <div className="cps-dial-prefix">
              <span className={`fi fi-${selected.code.toLowerCase()} cps-flag-img`}></span>
              <span className="cps-dial">{selected.dial}</span>
            </div>
          )}
          <input
            type="tel"
            className={`cps-phone-input ${selected ? 'has-prefix' : ''}`}
